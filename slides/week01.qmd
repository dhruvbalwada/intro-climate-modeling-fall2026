...
::: {.notes}
Flag the early project-title date (Oct 9!). Late policy: 10%/day, one free late HW. AI policy: encouraged + disclosed — one line in each submission; understanding is tested in class.
:::
...
::: {.notes}
Front-loads the setup pain while TAs are in the room instead of leaving it to students alone at home. TAs collect names of anyone without hub access.
:::
...
First stop: **Lab 0b — Python, just enough to start** (on the [Labs page](https://dhruvbalwada.github.io/intro-climate-modeling-fall2026/labs/)).

::: {.notes}
A map, not a tutorial — we learn the tools by using them.
:::
...
::: {.notes}
Abstract here; Lab 0c makes it concrete with NYC data shortly.
:::
...
::: {.notes}
The board collection is the point: same data, same model, different answers = parameter uncertainty, experienced firsthand. Then the punchline in part 4: ERA5 tracks the weather (r≈0.95), the free-running CMIP6 model ignores it (r≈0) — and that's by design; it only owes you the distribution. Discussion prompts are in the notebook.
:::
